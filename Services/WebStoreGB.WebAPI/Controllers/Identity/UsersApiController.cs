...
using WebStoreGB.Domain.DTO.Identity;
using WebStoreGB.Domain.Entities.Identity;
using WebStoreGB.Interfaces;

namespace WebStoreGB.WebAPI.Controllers.Identity
{
    [Route(WebAPIAddresses.Identity.Users)]
    [ApiController]
    public class UsersApiController : ControllerBase
    {
        //IdentityUser  если мы используем втроенные классы в Identity,  а не создаем свои(в нашем случае User,Role) то мы можем ипользовать классом UserStore, если мы используем свой класс User а роль стандартная то мы используем UserStore<свой класс>
        //IdentityRole  А в наешм случае, когда есть свой User и Role, мы используем UserStore<User, Role, WebStoreGBDB>
        private readonly UserStore<User, Role, WebStoreGBDB> _UserStore;
        public UsersApiController(WebStoreGBDB db)
        {
            _UserStore = new UserStore<User, Role, WebStoreGBDB>(db);
            //_UserStore.AutoSaveChanges = false; // если false то нам вручную придётся сохранять все данные, но нам всё равно придётся созранять вручную....
        }
        ...
        public async Task<string> SetUserNameAsync([FromBody] User user, string name)
        {
            ...
            return user.UserName; // возвращаем имя потому что система Identity может отказаться устанавливать имя пользователя и имя останется предыдущим
        }

        // Normal - нормализация, в данном случаем приведение имени пользователя к верхнему регистру, в верзнем ресгистре имена быстрее сравниваются 
