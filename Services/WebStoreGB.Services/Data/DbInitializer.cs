...
namespace WebStoreGB.Services.Data
{
    public class DbInitializer
    {
        ...
        private async Task InitializeProductsAsync()
        {
            if (_db.Sections.Any())
            {
                _Logger.LogInformation("Инициализая о товарах не требуется");
                return;
            }
            ...
            foreach (var product in TestData.Products)
            {
                ...
                if (product.BrandId is { } brand_id)  //Проверяем есть ли у товара бренд
            ...
            _Logger.LogInformation("Запись продуктов...");
            ...
            _Logger.LogInformation("Запись продуктов выполнена успешно");
        }
        ...
        private async Task InitializeIdentityAsync()
        {
            ...
            if (await _UserManager.FindByNameAsync(User.Administrator) is null)
            {
                ...
                else
                {
                    ...
                    _Logger.LogInformation($"Учётная запись администратора не создана ! Ошибки: {0}"
                        , string.Join(", ", errors));
